...
var LowBalanceWarning = function () {
    var dataTable = {}, obj = {}, queryB2iSmartFromsDaily = "queryPushBtoiSenceDetailByB2i_Type.view", queryB2iSmartFromsMonthly = "queryPushBtoiScenceStaticMonthByB2i_Type.view",
        downLoadB2iSmartFromsUrl = "downloadB2iSmartCloudFormData.view",b2iType = 1;
    ...
    obj.dailyDataTableInit = function (type) {
        var option = {
            ...
            columns: [
                {data: "timest", width: 400, title: "任务日期", className: "dataTableFirstColumns"},
                {data: "user_id", width: 300, title: "用户编码", className: "centerColumns"},
                {data: "phone", width: 800, title: "手机号", className: "centerColumns"},
                {data: "sale_name", width: 600, title: "活动名称", className: "centerColumns"},
    ...
    obj.monthlyDataTableInit = function (type) {
        var option = {
            ...
            columns: [
                ...
                {data: "topup_sum", width: 600, title: "累计充值余额", className: "centerColumns"}
            ]
        };
        dataTable = $plugin.iCompaignTable(option);
    };
...
// 加载余额不足提醒
function loadLowBalanceWarning(type) {
    LowBalanceWarning.initQueryData();

    if (type == 1) // 日报
    {
        LowBalanceWarning.dailyDataTableInit(1);
    }
    else if (type == 2) // 月报
    {
        LowBalanceWarning.monthlyDataTableInit(2);
    }

    LowBalanceWarning.initEvent(type);
