var policy_city = function () {
    ...
    obj.initEvent = function () {
        ...
        $(".addBtn").click(function () {
            obj.addOrEdit(policy_constant.operate.create, 0, '', 0);
        })
            ...
            var options = {
                ...
                columns: [
                    ...
                    {data: "expireTime", title: "结束时间", width: '10%'},
                    {
                        title: "操作", width: "12%",
                        render: function (data, type, row) {
                            var detailBtnHtml = "<a title='详情' class='btn btn-primary btn-preview' href='javascript:void(0)' onclick='policy_city.detailPolicy(\"{0}\")'><i class=\"fa fa-eye\"></i></a>".autoFormat(row.policyId);
                            var editBtnHtml = "<a title='修改' class='btn btn-info btn-edit'  onclick='policy_city.addOrEditPolicy(\"{0}\",{1})'><i class='fa fa-pencil-square-o'></i></a>".autoFormat(policy_constant.operate.update, row.policyId);
                            var deleteBtnHtml = "<a title='删除' class='btn btn-danger btn-delete' href='javascript:void(0)' onclick='policy_city.deletePolicy(\"{0}\",\"{1}\")'><i class=\"fa fa-trash-o\"></i></a>".autoFormat(row.policyId, row.policyName);
    ...
    obj.sliceTable = function (data, $table) {
        ...
        data.forEach(function (item, index) {
            ...
            html += "<a class='btn btn-primary btn-add' title='新增' onclick='policy_city.addOrEdit(\"{0}\",{1},\"{2}\",{3},{4},{5})'><i class='fa fa-plus'></i></a>".autoFormat(policy_constant.operate.create, item.menuId, item.menuName, item.pId, item.menuLevel, item.areaId);
            html += "<a class='btn btn-info btn-edit' title='修改' onclick='policy_city.addOrEdit(\"{0}\",{1},\"{2}\",{3},{4},{5})'><i class='fa fa-pencil-square-o'></i></a>".autoFormat(policy_constant.operate.update, item.menuId, item.menuName, item.pId, item.menuLevel, item.areaId);
            html += "<a class='btn btn-warning btn-below' title='进入' onclick='policy_city.intoDetail({0},{1},{2})'><i class='fa fa-chevron-down'></i></a>".autoFormat(item.menuId, item.pId, item.menuLevel);
            html += "<a class='btn btn-danger btn-delete' title='删除' hasDelete onclick='policy_city.delete({0},\"{1}\")' {2}><i class='fa fa-trash-o'></i></a>".autoFormat(item.menuId, item.menuName, item.policyNum > 0 ? "disabled" : "")
            html += "</td>";
        })
        $table.append(html);
