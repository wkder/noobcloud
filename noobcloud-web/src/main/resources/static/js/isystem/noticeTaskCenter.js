...
var noticeTaskCenter = function () {
    var getNoticeTaskCenterUrl = "getNoticeTaskCenterUrl.view";
    var dataTable = {};
    var obj = {};
            ...
            if (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i].id == areaCode){
                        $areaSelect.append("<option value='A' selected>B</option>".replace(/A/g, data[i].id).replace(/B/g, data[i].name));
                    }
                    else if(areaCode == 99999)
                    {
                        $areaSelect.append("<option value='A' >B</option>".replace(/A/g, data[i].id).replace(/B/g, data[i].name));
                    }
                }
    ...
    obj.dataTableInit = function () {
        ...
        var params = "status=" + $(".querySelectType").val()+"&executeTime="+$("#executeTime").val()+"&taskName="+$(".queryTaskName").val();
        var option = {
            ele: $("#dataTable"),
            ajax: {
                url: getNoticeTaskCenterUrl + "?" + params,
                type: "POST"
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var buttons = "";
                        var startBtnHtml = "<a title='启动'  class='startBtn btn btn-primary btn-edit btn-sm' href='javascript:void(0)' onclick='noticeTaskCenter.startItem(" + row.id + ",\"" + row.status + "\")' >启动</a>";
                        var suspendBtnHtml = "<a title='中断'  class='suspendBtn btn btn-danger btn-edit btn-sm' href='javascript:void(0)' onclick='noticeTaskCenter.suspendItem(" + row.id + ",\"" + row.status + "\")' >中断</a>";
                        var exportBtnHtml = "<a title='导出结果'  class='exportBtn btn btn-primary btn-edit btn-sm' href='javascript:void(0)' onclick='noticeTaskCenter.exportItem(" + row.taskId + ")' >导出结果</a>";
                        if (row.status == 2) {//待执行
                            // 中断功能暂未开放
    ...
    obj.evtOnRefresh = function () {
        ...
        dataTable.ajax.url(getnoticeTaskCenterUrl + "?" + params);
...
function onLoadBody() {
    noticeTaskCenter.initQueryRow();
    noticeTaskCenter.dataTableInit();
    noticeTaskCenter.initBtn();
}