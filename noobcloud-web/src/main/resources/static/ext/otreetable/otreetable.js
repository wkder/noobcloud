...
function oTreeTable(id, tb, _cfg) {
    ...
    // window.setTimeout(_.destroy, 2000, _)
}
...
oTreeTable.prototype._buildSwitch = function (rData, hasChild) {
    var arr = ['<img id="' + this._buildSwitchId(rData.id) + '"', ' src="' + this.config.iconPath + (hasChild ? (rData.isExpand ? 'minus.gif' : 'plus.gif') : 'empty.gif') + '"', ' class="otb-icon"', '/>',]
    return arr.join('')
}

oTreeTable.prototype._buildIcon = function (row, hasChild) {
    if (this.config.showIcon) {
        var arr = ['<img id="' + this._buildIconId(row.id) + '"', ' src="' + this.config.iconPath + (row.isExpand ? 'folderopen.gif' : 'folder.gif') + '"', ' class="otb-icon"', '/>']
        return arr.join('')
    }
    return ''
}
        ...
        if (arr[i].pid == pid) {
            ...
            for (var j = 0, c = arr[i].obj.cells.length; j < c; j++) {
                ...
                if (j == 0) {
                    ...
                    cell.lang = arr[i].id;
                    cell.innerHTML = strSpace + strSwitch + _._buildIcon(arr[i]) + '<span style="cursor: pointer;">' + arr[i].obj.cells[j].innerHTML + '</span>'
                } else {
...
oTreeTable.prototype.destroy = function (_) {
    _ = null;
}
